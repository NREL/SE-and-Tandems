...
def calc_tandem_4T_eff(top_file_list, bot_file_list, lambdafile, coupling, transmission): #top_file_list= SE file of top cells, bot_file_list=SE file of bottom cells, lambdafile= Transmission file of top cell, coupling= different optical coupling methods such as ideal, fixed and lambda, and transmission= controlling light transmission using fixed coupling onto bottom cells
    ...
    for filename1 in bot_file_list:
        ...
        for filename2 in top_file_list:
            ...
            if coupling == 'lambda':
                ...
                try:
                    ...
                    for filename3 in lambdafile:
                        ...
                        transmission_name = clip_name(filename3).replace('_transmission','')
            ...
            #ax2.legend(bbox_to_anchor=(1.05, 1), loc='upper left', bbox_inches='tight', borderaxespad=0.)
            ax2.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=2)
...
#                
#                if coupling == 'lambda':
#                    tandem_line = [top_name, top_eff, bot_name, bot_eff, tandem_eff,
#                              coupling, transmission]
#                    tandem_list.append(tandem_line)
#                    
#                else:
#                    tandem_line = [top_name, top_bg_nm, top_eff, bot_name, bot_eff, tandem_eff,
#                              coupling, transmission]
#                    tandem_list.append(tandem_line)
            ...
            if print_graphs == 1:
                ...
                ax1.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=2)
                ax1.set_ylim(0, 100)
                ax3.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=2)
                ax3.set_ylim(0, 70)
                ax4.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=2)
...
def calc_tandem_2T_eff(top_file_list, bot_file_list, lambdafile, coupling, transmission): #top_file_list= SE file of top cells, bot_file_list=SE file of bottom cells, lambdafile= Transmission file of top cell, coupling= different optical coupling methods such as ideal, fixed and lambda, and transmission= controlling light transmission using fixed coupling onto bottom cells
    ...
    for filename1 in bot_file_list:
        ...
        for filename2 in top_file_list:
            ...
            #if coupling != 'lambda':
            #    ax2.plot(bot_se['wl'], bot_se['QE'], label="bottom filtered QE " + bot_name)
            #if bot_name not in cellNameList:
            ax2.plot(bot_se['wl'], bot_se['QE'], label=bot_name + " filtered through " + top_name)
            ...
            #plotting
#            if top_name not in cellNameList:
#                ax5.plot(new_top_v, new_i, label=("top new I-V " + top_name))



#            if bot_name not in cellNameList and top_name not in cellNameList:
            ax5.plot(added_voltage, new_i, label=("tandem new I-V " + top_name + " and " + bot_name))

          #  if bot_name not in cellNameList:
#commented                ax5.plot(bot_se.v, filtered_bot_i, label=("filtered bottom I-V " + bot_name))
#                ax5.plot(bot_se.v, bot_se.i, label=("raw bottom " + bot_name))
#            ax6.plot(new_bot_v, new_i, label=("filtered bottom new I-V " + bot_name))                                  #issue here for bottom cell 2
...
#                ax6.plot(bot_se.v, bot_se.i, label=("raw bottom " + bot_name)) #.replace(0,np.nan,inplace=True) not allowed, None value not allowed   same for .replace(0,np.max(top_se.i),inplace=True) # .replace(0,'') throws error
            ax6.plot(new_bot_v, new_i, label=(bot_name + " filtered through " + top_name))                                  #issue here for bottom cell 2
            if top_name not in cellNameList:
                ax6.plot(top_se.v.replace(0,), top_se.i.replace(0,), label=("raw top " + top_name)) 
#                ax6.plot(top_se.v, top_se.i, label=("raw top " + top_name)) 
#                top_xlim.append(np.max(top_i))
            
#            ax6.aspect('auto')
     #       ax6.set_ylim(0, )  #np.max(top_xlim)*1.01
     #       ax6.set_xlim(0, )
...
#                
#                if coupling == 'lambda':
#                    tandem_line = [top_name, top_2T_eff, bot_name, bot_2T_eff, Effi_2T,
#                                coupling, transmission]
#                    tandem_list.append(tandem_line)
#                    
#                else:
#                    tandem_line = [top_name, top_bg_nm, top_2T_eff, bot_name, top_2T_eff, Effi_2T,
#                                coupling, transmission]
#                    tandem_list.append(tandem_line)
            ...
            if print_graphs == 1:
                ...
                #this section needs to change if multiple transmission files are allowed -> get rid of if/elif and leave one line of ax1
                #if coupling != 'lambda':
                ax1.plot(bot_se['wl'], bot_se['2Teffic'], label=bot_name+' filtered through '+top_name)
                #if bot_name not in cellNameList and top_name not in cellNameList:
                    #ax1.plot(bot_se['wl'], bot_se['2Teffic'], label=bot_name)
                #if (bot_name not in cellNameList and top_name not in cellNameList) or (bot_name in cellNameList and top_name in cellNameList):
                ax3.plot(bot_se['wl'], bot_se['2TSE'], '--', label=bot_name+' filtered through '+top_name) #XXXXXXXX
                ...
                if bot_name not in cellNameList:
                    #ax1.plot(bot_se['wl'], bot_se['2Teffic'], label=bot_name)
                    #ax3.plot(bot_se['wl'], bot_se['2TSE'], '--', label=bot_name)
                    ax4.plot(bot_se['wl'], bot_se['1JSE'], '--', label=bot_name)
